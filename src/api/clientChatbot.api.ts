...
export const getChatbotThread = () =>
  useMutation({
    mutationFn: () =>
      apiClient.post(`/api/chatbot/threads`).then(unwrapAxiosResponse),
  });
...
export const askQuestion = () =>
  useMutation({
    mutationFn: (params: Question) =>
      apiClient
        .post(`/api/chatbot/ask`, params)
        .then(unwrapAxiosResponse)
        .then((response) => {
          console.log("This is the ask question response", response);
          return response; // Return the response
        })
        .catch((err) => console.log("Error sending question:", err)),
  });
    ...
    mutationFn: async (params: {
      ...
      if (params.userType === "admin" || params.userType === "lawyer") {
        ...
        try {
          ...
          console.error(
            "Error with admin message endpoint:",
      ...
      try {
        ...
        if (
          ...
          if (
            ...
            endpoint.includes("/lawyer/")
          ) {
            console.log(
              "Falling back to regular endpoint due to permission error"
            );

            try {
              // Try the regular (non-lawyer) endpoint
              const fallbackResponse = await apiClient.post(
                `/api/chatbot/lawyer-chat/send-message/${params.threadId}`,
                {
                  message: params.message,
                }
              );
    ...
    mutationFn: async ({
      ...
      try {
        ...
        const response = await fetch(
          `${process.env.REACT_APP_API_BASE_URL || ""}api/chatbot/ask/stream`,
          {
            method: "POST",
            headers: {
...
export const uploadFile = () =>
  useMutation({
    mutationFn: async (params: { threadId: string; file: File }) => {
      ...
      const response = await apiClient.post(
        `/api/chatbot/upload?&threadId=${params.threadId}`,
        formData,
        {
          headers: {
  ...
  export const useRequestALawyer = (options: { 
    onSuccess?: () => void; 
    ...
    return useMutation({
      mutationFn: async (params: { threadId: string }) => {
        const response = await apiClient.post(
          `/api/chatbot/lawyer-chat/request/${params.threadId}`,
          {}, // Empty body object
          {
            headers: {
              "Content-Type": "application/json",
...
export function getCategoriesAndChats(key: string) {
  return useQuery({
    ...
    queryFn: async ({ signal }) => {
      const response = await apiClient.get("/api/chatbot/threads/all", {
        signal,
      });
      ...
      if (data.uncategorized && data.uncategorized.threads) {
        data.uncategorized.threads.sort(
          (a: any, b: any) =>
...
export function getCategories(key: string) {
  return useQuery({
    queryKey: [key],
    queryFn: async ({ signal }) => {
      const response = await apiClient.get("/api/chatbot/categories", {
        signal,
      });
      return unwrapAxiosResponse(response);
    },
...
export function getLawyerChats(
  key: string,
  companyId: string,
  adminId: string
) {
