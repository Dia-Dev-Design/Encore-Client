...
const HistoryCategory: React.FC<HistoryChatProps> = ({
  ...
  return (
    <div className="w-full flex flex-col justify-between items-center">
      <button
        className={`w-full min-h-10 flex flex-row gap-x-2 px-2 items-center ${
          isOpen ? "bg-primaryLinkWater-200 rounded-lg my-1" : ""
        }`}
        onClick={() => handleCategoryOpen()}
      >
        <img className="brightness-0 invert" src={FolderIcon} alt="Category" />
        <p
          className={`w-full text-base font-medium font-figtree text-left pl-2 ${
            isOpen ? "text-neutrals-black" : "text-neutrals-white"
          }`}
        >
          {threadInfo.name}
        </p>
      ...
      {isOpen && (
        <div className="w-full flex flex-col gap-y-2 justify-between items-center bg-primaryLinkWater-50 rounded-lg py-2 transition">
          {threadsInCategoryData && threadsInCategoryData.length > 0 ? (
            threadsInCategoryData.map((chat: any) => (
            ...
            <p className="font-figtree font-medium text-greys-700">
              No chats available
            </p>
          )}
        </div>
