...
const ChatSection: React.FC<ChatSectionProps> = ({
  ...
  useEffect(() => {
    console.log("This is the chatbot thread in useEffect", chatId);
    ...
    const messageChannel = subscribeToMessages(chatId);
      ...
      uploadFileToChat(payload, {
        onSuccess: (data: any) => {
          ...
          const newDocumentNode: HistoryNode = {
            ...
            role: "document",
            url: data.url,
            forLawyer: chatbotThreadType === ChatTypeEnum.ChatLawyer,
          };
          ...
          handleQuestion(
            question,
            chatbotThreadId,
            chatbotThreadType && chatbotThreadType === ChatTypeEnum.ChatLawyer,
            data.id
          );
        },
        onError: (error: any) => {
  ...
  const handleQuestion = (
    question: string,
    fetchedThread?: string,
    isForALawyer?: boolean,
    file_Id?: string
  ) => {
      ...
      function sendQuestionForLawyer() {
        ...
        sendQuestionWithFile(payload, {
          onSuccess: (data) => {
            setSelectedChatType(ChatSpaceType.chatSpace);
            invalidateMainQueries();
            const foundNode = historyConversation.find(
            ...
            if (!foundNode) {
              ...
              // setIsGettingLawyer(true)
              setChatId(payload.thread_id);
              // setTimeout(() => {
              //     setIsGettingLawyer(false)
              // }, 2000)
              console.log(
                "This is the payload when sendind a question to an attorney======XXXX",
                payload
              );
