...
const AdminLayout: React.FC<AdminLayoutProps> = ({
  isUser, isDarkBackground = false, title,
  user, children, options, isSideBarCollapsed,
  setIsSideBarCollapsed, notificationBadgeCounter,
  hideNotifications
}) => {
  return (
    <div className="flex flex-col md:min-h-screen">
      <AdminHeader isUser={isUser} user={user} notificationBadgeCounter={notificationBadgeCounter} hideNotifications={hideNotifications}/>
      <div className="flex flex-1 h-max md:h-auto relative">
          ...
          {!isSideBarCollapsed && 
            <div className="hidden md:flex flex-row justify-start w-full mt-4 mb-5">
              <img
                src={EncoreIcon}
                alt="Logo"
                className="w-auto pl-7 h-6"
              />
            </div>
          }

          <Tooltip title={isSideBarCollapsed ? "Expand Menu" : "Minimize Menu"} placement="right">
            <button
              className={`${!isSideBarCollapsed ? "hidden md:flex absolute top-[80px] right-3" : " mt-4 mb-2"} `}
              onClick={() => setIsSideBarCollapsed(!isSideBarCollapsed)}
            >
              <img src={isSideBarCollapsed ? ExpandIcon : CollapseIcon} alt="Logo" className="h-6" />
            </button>  
          </Tooltip>
        ...
        >
          {title && (
            <h1 className="hidden md:flex h-20 text-3xl font-medium px-8 text-primaryMariner-900 fixed w-full 
              pt-10 pb-14 bg-neutrals-white border-y border-greys-300 z-30 font-figtree">
              {title}
            </h1>
          )}
          <div className={clsx("content-dashboard", { "pt-6 md:pt-24": title })}>
            {children}
          </div>
        </main>
      </div>
