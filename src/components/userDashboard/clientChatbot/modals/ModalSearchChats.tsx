...
interface ModalSearchChatsProps {
    categoriesAndChats: any;
    isModalOpen: boolean;
    closeModal: () => void;
    loadPreviousChat: (chat: ChatbotThread, title: string) => void;
}

const ModalSearchChats: React.FC<ModalSearchChatsProps> = ({
    categoriesAndChats,
    isModalOpen,
    closeModal,
    loadPreviousChat,
    ...
    const [allThreads, setAllThreads] = useState<ChatbotThread[]>([]);
    ...
    useEffect(() => {
        const updateList = () => {
            if (categoriesAndChats) {
                const threads: ChatbotThread[] = [];

                categoriesAndChats.categorized.forEach((categorizedChat: any) => {
                    if (Array.isArray(categorizedChat.threads)) {
                        threads.push(...categorizedChat.threads);
                    }
                });

                if (Array.isArray(categoriesAndChats.uncategorized.threads)) {
                    threads.push(...categoriesAndChats.uncategorized.threads);
                }
        ...
        updateList();
    }, [isModalOpen, categoriesAndChats]);
...
export default ModalSearchChats;
