...
import { BasicInfoData, CountryData, IndustryData, User } from "utils/interfaces";
...
const BasicInfo: React.FC<BasicInfoProps> = ({ basicInfoData, setBasicInfoData, basicInfoErrors }) => {
    ...
    const [selectedIndustry, setSelectedIndustry] = useState<IndustryData>({id: "", name:""});
    const [fullNameErrorMessage, setFullNameErrorMessage] = useState<string>("");
    const [countryCodeErrorMessage, setCountryCodeErrorMessage] = useState<string>("");
    const [phoneNumberErrorMessage, setPhoneNumberErrorMessage] = useState<string>("");
    const [companyNameErrorMessage, setCompanyNameErrorMessage] = useState<string>("");
    const [industryTypeErrorMessage, setIndustryTypeErrorMessage] = useState<string>("");
    ...
    const [selectedCountry, setSelectedCountry] = useState<CountryData>(countries.find(industry => industry.code === "US") || { code: "US", name: "United States", callingCode: "+1" });
    const toggleCountryCodeDropdown = () => setIsCountryCodeOpen(!isCountryCodeOpen);
    const [isCompanyStatesOpen, setIsCompanyStatesOpen] = useState<boolean>(false);
    ...
    useEffect(() => {
        ...
        if (jsonUser){
            ...
            getIndustriesData(tempUser.accessToken);
            if (jsonCompany){
                const tempCompany = JSON.parse(jsonCompany)
                getStep1Data(tempUser.accessToken, tempCompany.id);
            } else {
    ...
    const getStep1Data = async (accessToken: string, companyId: string): Promise<boolean> => {
        try {
            const response = await fetch(process.env.REACT_APP_API_BASE_URL+"api/register/step1/"+companyId, {
                method: "GET", 
            ...
            setPhoneNumber(dataReceived.phone.replace("+", "").replace(selectedCountry.callingCode, ""));
            const industry = industries.find(industry => industry.id === dataReceived.industryId);
    ...
    const getIndustriesData = async (accessToken: string): Promise<boolean> => {
        try {
            ...
            const dataReceived: IndustryData[] = await response.json();
    ...
    const handleIndustryChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        setSelectedIndustry(industries.find(industry => industry.id === e.target.value) || {id: "", name: ""});
    ...
    const handlePhoneNumberChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        {
            if (selectedCountry){
                if (isValidPhoneNumber(selectedCountry.callingCode+e.target.value, selectedCountry.code)) {
                    setPhoneNumberErrorMessage("");
                    setBasicInfoData({ ...basicInfoData, phone: "+"+selectedCountry.callingCode + e.target.value })
                } else {
    ...
    const handleClickOutside = (event: MouseEvent) => {
        if (dropdownCountryCodeRef.current && !dropdownCountryCodeRef.current.contains(event.target as Node)) {
            setIsCountryCodeOpen(false);
        }
    ...
    return (
        <S.Container>
            <S.TitleContainer>
                ...
                <S.EmailRegistered>Your registered email: <span>{userData?.email}</span></S.EmailRegistered>
            </S.TitleContainer>
            <S.Form>
                <S.FormGroup>
                    <S.Label>Full Name <span className="required">*</span></S.Label>
                    <S.Input
                        ...
                        onChange={(e) => setBasicInfoData({ ...basicInfoData, fullname: e.target.value })} 
                    />
                    {fullNameErrorMessage && fullNameErrorMessage != "" && <S.ErrorLabel>{fullNameErrorMessage}</S.ErrorLabel>}
                </S.FormGroup>
                <S.FormRowGroup>
                    <S.NarrowFormGroup>
                        <S.Label>Country Code <span className="required">*</span></S.Label>
                        <S.DropdownContainer>
                            <S.DropdownHeader ref={dropdownCountryCodeRef} onClick={toggleCountryCodeDropdown} hasErrors={countryCodeErrorMessage != ""} >
                                {selectedCountry ? (
                                    <S.DropdownOption>
                                        <S.SelectedFlag src={`https://flagcdn.com/h20/${selectedCountry.code.toLowerCase()}.png`} alt={`${selectedCountry.name} flag`} />
                                        {selectedCountry.code}
                                    </S.DropdownOption>
                                ) : (
                                    <S.DropdownOption>Select</S.DropdownOption>
                                )}
                                <S.SelectArrow src={arrow} flipped={isCountryCodeOpen ? true : false}/>
                            </S.DropdownHeader>

                            {isCountryCodeOpen && (
                                <S.DropdownList>
                                {countries.map((country) => (
                                    <S.DropdownItem key={country.code} onClick={() => handleSelectCountryCode(country)}>
                                    <S.ItemFlag src={`https://flagcdn.com/h20/${country.code.toLowerCase()}.png`}  alt={`${country.name} flag`} />
                                        {country.name}
                                    </S.DropdownItem>
                                ))}
                                </S.DropdownList>
                        ...
                        {countryCodeErrorMessage && countryCodeErrorMessage != "" && <S.ErrorLabel>{countryCodeErrorMessage}</S.ErrorLabel>}
                    </S.NarrowFormGroup>

                    <S.WideFormGroup>
                        <S.Label>Phone Number <span className="required">*</span></S.Label>
                        <S.Input
                        ...
                        />
                        {phoneNumberErrorMessage && phoneNumberErrorMessage != "" && <S.ErrorLabel>{phoneNumberErrorMessage}</S.ErrorLabel>}
                    </S.WideFormGroup>
                </S.FormRowGroup>
                <S.FormGroup>
                    <S.Label>Company Name <span className="required">*</span></S.Label>
                    <S.Input
                        ...
                        onChange={(e) => setBasicInfoData({ ...basicInfoData, companyName: e.target.value })} 
                    />
                    {companyNameErrorMessage && companyNameErrorMessage != "" && <S.ErrorLabel>{companyNameErrorMessage}</S.ErrorLabel>}
                </S.FormGroup>
                <S.FormGroup>
                    <S.Label>Industry type <span className="required">*</span></S.Label>
                    <S.NormalDropdown value={selectedIndustry.id} onChange={handleIndustryChange} hasErrors={industryTypeErrorMessage != ""}>
                        <S.NormalOption value="" disabled>Select an Industry</S.NormalOption>
                    ...
                    {industryTypeErrorMessage && industryTypeErrorMessage != "" && <S.ErrorLabel>{industryTypeErrorMessage}</S.ErrorLabel>}
                </S.FormGroup>
                <S.FormGroup>
