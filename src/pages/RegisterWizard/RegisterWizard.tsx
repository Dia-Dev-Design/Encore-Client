...
const RegisterWizard: React.FC = () => {
  ...
  const saveStepData = async (
    ...
    try {
      ...
      if (currentStep === 0) {
        setLocalItemWithExpiry(
          "company",
          JSON.stringify(parseCompany(dataReceived)),
          2
        );
  ...
  const handleErrors = (errorMessage: string) => {
    switch (currentStep) {
      case 0:
        let basicInfoErrors = [""];
        if (
          errorMessage.includes("fullname should not be empty") ||
          errorMessage.includes("fullname must be a string")
        ) {
          basicInfoErrors.push("fullname");
        }
        if (
          errorMessage.includes("phone should not be empty") ||
          errorMessage.includes("phone must be a valid phone number")
        ) {
  ...
  const goNext = async () => {
    ...
    if (currentStep === 3) {
      ...
      if (currentStep > 0) {
        updatedEndpoint =
          process.env.REACT_APP_API_BASE_URL + endpoint + companyData?.id;
      } else {
  ...
  const validateCompanyDetails = (): boolean => {
    ...
    if (
      companyDetailsData.hasW2Employees &&
      (!companyDetailsData.employeesStates ||
        companyDetailsData.employeesStates.length === 0)
    ) {
  ...
  return (
    <S.Container>
      ...
      <S.RightContainer>
        ...
        <S.Buttons>
          {currentStep > 0 && (
            <S.BaseButton onClick={goBack}>Back</S.BaseButton>
          )}
          {visibleConfirm && (
